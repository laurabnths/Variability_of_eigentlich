...
dtf = pd.concat(nl, ignore_index=True)
...
############################################################
newbigr_davor = [] # list with word before eigentlich and eigentlich variation
newbigr_danach = [] # list with word after eigentlich and eigentlich variation
for hallo in all_bigrams_wordssegments:
    if hallo[0][0] == "eigentlich":
        newbigr_danach.append((hallo[1][0], hallo[0][1]))
    if hallo[0][1] == "eigentlich":
        newbigr_davor.append((hallo[0][0], hallo[1][1]))

Count_bigr_davor = Counter(newbigr_davor)
Count_bigr_danach = Counter(newbigr_danach)

newtrigr_davor = []
newtrigr_davordanach = []
newtrigr_danach = []
...
for wrt in word_davor_2:
    word_count_davor_2.append(dict_cminus2[wrt]) #Frequenz von Wörtern, die zwei vor eigentlich sind

word_count_davor_1 = [] # Frequenz von den Wörtern, die vor eigentlich sind
for wrt in word_davor_1:
    word_count_davor_1.append(word_counts[wrt])

word_count_danach_1 = [] # Frequenz von den Wörtern, die nach eigentlich sind
for wt in word_danach_1:
    word_count_danach_1.append(word_counts[wt])

word_count_danach_2 = []
for wt in word_danach_2:
    word_count_danach_2.append(dict_cplus2[wt]) #Frequenz von Wörtern, die zwei nach eigentlich sind

############ lists with canonIPA für A2, A1, C1, C2 #############
...
######## morphTag (part of speech) for A2, A1, C1, C2 ################################
...
len_difference = [] # difference between CanonIPA and segments
...
#print(speak_duration)

######################################################################

dtf_segm_column = list(dtf_eig['segment'])
cnt_segm = Counter(dtf_segm_column) # print(len(cnt_segm)) --> 168
...
for hdf, hdf1 in davor_entropy.items():
    davor_entropy[hdf] = entropy(hdf1, qk=None, base=2)

davor_entropy_list = [davor_entropy[shit] for shit in word_davor_1] # entropy list for csv
...
for hdf2, hdf3 in danach_entropy.items():
    danach_entropy[hdf2] = entropy(hdf3, qk=None, base=2)

after_entropy_list = [danach_entropy[shit2] for shit2 in word_danach_1] # list for csv file
...
for hallo1 in all_bigrams_wordssegments:
    if hallo1[0][0] == "eigentlich":
        seg_bigram_danach.append((hallo1[1][0], hallo1[1][1]))
    if hallo1[0][1] == "eigentlich":
        seg_bigram_davor.append((hallo1[1][0], hallo1[1][1]))


newlist1 = [] # list with last segments before eigentlich variation !!
for holy in seg_bigram_davor:
    if holy[0] in extra_exceptions:
        newlist1.append((holy[0], holy[1]))
    elif holy[0][-1] in suprasegmental:
        newlist1.append((holy[0][-2], holy[1]))
    else:
        newlist1.append((holy[0][-1], holy[1]))
...
for holysh in seg_bigram_danach:
    if holysh[1] in extra_exceptions:
        newlist2.append((holysh[0], holysh[1]))
    elif holysh[1][0] in suprasegmental:
        newlist2.append((holysh[0], holysh[1][1]))
    else:
        newlist2.append((holysh[0], holysh[1][0]))
...
ADTF = pd.read_csv('A.csv', sep=',')


########################################################################################################################

df2 = pandas.DataFrame(data={'prob that before-segment occurs at least once in variation': prob_before_segm_in_vari,
                             'prob that after-segment occurs at least once in variation': prob_after_segm_in_vari,
                             'prob that preceding segment is a front_vowel and in variation': prob_davorfrontvowel_in_vari,
                             'prob that preceding segment is a central_vowel and in variation': 'None',
                             'prob that preceding segment is a back_vowel and in variation': 'None',
                             'prob that preceding segment is a voiced and in variation': prob_davorvoiced_in_vari,
                             'prob that preceding segment is a voiceless and in variation': prob_davorvoiceless_in_vari,
                             'prob that preceding segment is a plosive and in variation': prob_davorplosive_in_vari,
                             'prob that preceding segment is a nasal and in variation': prob_davornasal_in_vari,
                             'prob that preceding segment is a fricative and in variation': prob_davorfricative_in_vari,
                             'prob that preceding segment is a lateral and in variation': prob_davorlateral_in_vari,
                             'prob that preceding segment is a trill and in variation': 'None',
                             'prob that preceding segment is an approximant and in variation': 'None',
                             'prob that following segment is a front_vowel and in variation': prob_danachfrontvowel_in_vari,
                             'prob that following segment is a central_vowel and in variation': prob_danachcentralvowel_in_vari,
                             'prob that following segment is a back_vowel and in variation': 'None',
                             'prob that following segment is a voiced and in variation': prob_danachvoiced_in_vari,
                             'prob that following segment is a voiceless and in variation': prob_danachvoiceless_in_vari,
                             'prob that following segment is a plosives and in variation': prob_danachplosive_in_vari,
                             'prob that following segment is a nasal and in variation': prob_danachnasal_in_vari,
                             'prob that following segment is a fricative and in variation': prob_danachfricative_in_vari,
                             'prob that following segment is a lateral and in variation': 'None',
                             'prob that following segment is a trill and in variation': 'None',
                             'prob that following segment is an approximant and in variation': prob_danachapproximant_in_vari}, index=[0])
df2.to_csv("AssimilationProbs.csv", sep=',', index=True)